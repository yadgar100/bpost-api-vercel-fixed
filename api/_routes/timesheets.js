...
// ============================================
// GET /api/timesheets/employee/:employeeId - Get timesheets for specific employee
// ============================================
...
// ============================================
// GET /api/timesheets/pending - Get pending timesheets only (Admin approval queue)
// ============================================
    ...
    try {
        ...
        if (!employeeId || !locationId || !date || !checkInTime) {
            return res.status(400).json({
                success: false,
                error: 'Missing required fields: employeeId, locationId, date, checkInTime'
            });
        }
        ...
        const result = await pool.request()
            ...
            .query(`
                ...
                (EmployeeId, LocationId, Date, CheckInTime, CheckOutTime, 
                 RegularHours, OvertimeHours, Notes, GPSCheckIn, GPSCheckOut, Status)
                OUTPUT INSERTED.*
                VALUES 
                (@employeeId, @locationId, @date, @checkInTime, @checkOutTime,
                 @regularHours, @overtimeHours, @notes, @gpsCheckIn, @gpsCheckOut, 'pending')
            `);
...
// ============================================
// PUT /api/timesheets/:id - Update timesheet (Approve/Reject or Edit)
// ============================================
router.put('/timesheets/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (checkOutTime) {
            updateFields.push('CheckOutTime = @checkOutTime');
            request.input('checkOutTime', sql.Time, checkOutTime);
        }
        
        if (regularHours !== undefined) {
            updateFields.push('RegularHours = @regularHours');
            request.input('regularHours', sql.Decimal(5,2), regularHours);
        }
        
        if (overtimeHours !== undefined) {
            updateFields.push('OvertimeHours = @overtimeHours');
            request.input('overtimeHours', sql.Decimal(5,2), overtimeHours);
        }
...
// ============================================
// POST /api/timesheets/:id/approve - Quick approve endpoint
// ============================================
...
// ============================================
// POST /api/timesheets/:id/reject - Quick reject endpoint
// ============================================
