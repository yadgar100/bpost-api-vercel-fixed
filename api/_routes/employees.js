...
router.put('/employees/:id', async (req, res) => {
    try {
        const { firstName, lastName, email, department, position, hourlyRate, assignedLocations, isActive, country, currency, standardHours, overtimeRate, minimumHours, isAdmin, adminPermissions, password, branches } = req.body;
        const pool = await req.app.locals.getPool();
        let updateFields = [];
        ...
        if (branches !== undefined) { updateFields.push('Branches = @branches'); request.input('branches', sql.NVarChar(500), typeof branches === 'string' ? branches : JSON.stringify(branches || [])); }
